...
public class ProjectUpdateCommand implements Command {
  ...
  public void execute(PrintWriter out, BufferedReader in) {
    ...
    String title = Prompt.inputString(
        String.format("프로젝트명(%s)? ", out, in, project.getTitle()));
    String content = Prompt.inputString(
        String.format("내용(%s)? ", out, in, project.getContent()));
    Date startDate = Prompt.inputDate(
        String.format("시작일(%s)? ", out, in, project.getStartDate()));
    Date endDate = Prompt.inputDate(
        String.format("종료일(%s)? ", out, in, project.getEndDate()));

    String owner = null;
    while (true) {
      String name = Prompt.inputString(
          String.format("만든이(%s)?(취소: 빈 문자열) ", out, in, project.getOwner()));
      if (name.length() == 0) {
    ...
    while (true) {
      String name = Prompt.inputString(
          String.format("팀원(%s)?(완료: 빈 문자열) ", out, in, project.getMembers()));
      if (name.length() == 0) {
        break;
      } else if (memberListCommand.findByName(name) != null) {
        if (members.length() > 0) {
