...
public class Exam0130 {
  public static void main(String[] args) {
    ...
    // - 기계어로 바뀔 때 b = 11;로 저장된다 => 최적화 (Optimizing)
    ...
    //z = x + y; // 컴파일 오류! - 바이트 변수를 초과할 수 있기 때문
    
    int a = 5, c = 6, d;
    a = c;
    d = a + c;
    // 이렇게 int 타입으로 해줘야 한다. 왜냐하면 자바에서 
    // 정수의 기본 타입은 int이기 때문이다.
    
        // 컴파일이란 파일 전체를 한번에 기계어로 바꾸는게 아니라
    // 한 문장식 컴파일을 한다.
    ...
    s3 = s1; // OK!
    s3 = s2; // OK!
    //s3 = s1 + s2; // 컴파일 오류! - 연산을 수행하기 전에 4바이트로 
    //                  자동 연산된다. s1 + s2 모두 int로 전환됨
    //                  따라서 s3를 int로 바꿔야 한다.
