...
public class Exam0410 {
  ...
  static class SmartPhone {
    ...
    // 제품을 만든 후 사용하기 전에 구동되는데 문제가 없도록 기본값으로 설정되어야 한다.
    // 그래서 인스턴스를 사용하기 전에 적절한 값으로 초기화시키기 위해
    // 자동으로 호출되는 메소드를 도입하게 되었다.
    // 모든 클래스는 반드시 한 개 이상의 생성자를 가져야 한다.

    // 생성자 문법
    // - 클래스명(파라미터,...) {}
    // - 생성자를 선언할 때는 return타입을 지정하지 않는다.
    ...
    //      => public 으로 공개된다.
    //         예) public 클래스명() {}
    // 이 클래스에 생성자를 정의하지 않으면 다음과 같이 기본생성자가 추가된다.
    // => public SmartPhone() {}
  ...
  public static void main(String[] args) {
   ...
   //SmartPhone obj1 = new SmartPhone; 컴파일 오류
    ...
    //SmartPhone obj1 = new SmartPhone(100); 컴파일 오류 - int 파라미터를 가지는 생성자가 없다.
