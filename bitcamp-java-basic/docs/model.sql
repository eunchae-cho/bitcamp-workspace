...
-- 강사
DROP TABLE IF EXISTS lms_tschr RESTRICT;

-- 매니저
DROP TABLE IF EXISTS lms_mgr RESTRICT;

-- 교육과정
DROP TABLE IF EXISTS lms_lec RESTRICT;
...
-- 전공
DROP TABLE IF EXISTS lms_maj RESTRICT;
...
-- 교육과정강사
DROP TABLE IF EXISTS lms_lec_tchr RESTRICT;

-- 교실사진
DROP TABLE IF EXISTS lms_rpho RESTRICT;

-- 직급
DROP TABLE IF EXISTS lms_posi RESTRICT;

-- 부서
DROP TABLE IF EXISTS lms_dep RESTRICT;
...
CREATE TABLE lms_stud (
  ...
  work   VARCHAR(5)  NOT NULL COMMENT '재직여부', -- 재직여부
  ...
  bankno INTEGER     NULL     COMMENT '은행번호' -- 은행번호
)
COMMENT '학생';
...
-- 강사
ALTER TABLE lms_tschr
  ADD CONSTRAINT PK_lms_tschr -- 강사 기본키
    PRIMARY KEY (
      tno -- 강사번호
    );
...
-- 교육과정
CREATE TABLE lms_lec (
  lno     INTEGER      NOT NULL COMMENT '교육과정번호', -- 교육과정번호
  tilte   VARCHAR(255) NOT NULL COMMENT '과정명', -- 과정명
...
-- 교육과정
ALTER TABLE lms_lec
  ADD CONSTRAINT PK_lms_lec -- 교육과정 기본키
    PRIMARY KEY (
      lno -- 교육과정번호
    );
...
-- 전공
ALTER TABLE lms_maj
  ADD CONSTRAINT PK_lms_maj -- 전공 기본키
    PRIMARY KEY (
      majno -- 전공번호
    );

-- 전공 인덱스
CREATE INDEX IX_lms_maj
  ON lms_maj( -- 전공
    title ASC -- 전공명
  );
...
CREATE TABLE lms_bank (
  bankno INTEGER     NOT NULL COMMENT '은행번호', -- 은행번호
...
-- 교육과정강사
ALTER TABLE lms_lec_tchr
  ADD CONSTRAINT PK_lms_lec_tchr -- 교육과정강사 기본키
    PRIMARY KEY (
      lno, -- 교육과정번호
      tno  -- 강사번호
    );
...
-- 교실사진
CREATE TABLE lms_rpho (
  phono INTEGER      NOT NULL COMMENT '교실사진번호', -- 교실사진번호
  photo VARCHAR(255) NOT NULL COMMENT '사진파일', -- 사진파일
  rno   INTEGER      NOT NULL COMMENT '교실번호' -- 교실번호
)
COMMENT '교실사진';

-- 교실사진
ALTER TABLE lms_rpho
  ADD CONSTRAINT PK_lms_rpho -- 교실사진 기본키
    PRIMARY KEY (
      phono -- 교실사진번호
    );
...
-- 부서
ALTER TABLE lms_dep
  ADD CONSTRAINT PK_lms_dep -- 부서 기본키
    PRIMARY KEY (
      dno -- 부서번호
    );
...
-- 학생
ALTER TABLE lms_stud
  ADD CONSTRAINT FK_lms_maj_TO_lms_stud -- 전공 -> 학생
    FOREIGN KEY (
      majno -- 전공번호
    )
    REFERENCES lms_maj ( -- 전공
      majno -- 전공번호
    );
...
-- 강사
ALTER TABLE lms_tschr
  ADD CONSTRAINT FK_lms_user_TO_lms_tschr -- 사용자 -> 강사
    FOREIGN KEY (
      tno -- 강사번호
    )
    REFERENCES lms_user ( -- 사용자
...
-- 강사
ALTER TABLE lms_tschr
  ADD CONSTRAINT FK_lms_edu_level_TO_lms_tschr -- 학력 -> 강사
    FOREIGN KEY (
      elno -- 학력번호
    )
    REFERENCES lms_edu_level ( -- 학력
...
-- 강사
ALTER TABLE lms_tschr
  ADD CONSTRAINT FK_lms_maj_TO_lms_tschr -- 전공 -> 강사
    FOREIGN KEY (
      majno -- 전공번호
    )
    REFERENCES lms_maj ( -- 전공
...
-- 강사
ALTER TABLE lms_tschr
  ADD CONSTRAINT FK_lms_school_TO_lms_tschr -- 학교 -> 강사
    FOREIGN KEY (
      schno -- 학교번호
    )
    REFERENCES lms_school ( -- 학교
...
-- 강사
ALTER TABLE lms_tschr
  ADD CONSTRAINT FK_lms_bank_TO_lms_tschr -- 은행 -> 강사
    FOREIGN KEY (
      bankno -- 은행번호
    )
    REFERENCES lms_bank ( -- 은행
...
-- 매니저
ALTER TABLE lms_mgr
  ADD CONSTRAINT FK_lms_dep_TO_lms_mgr -- 부서 -> 매니저
    FOREIGN KEY (
      dno -- 부서번호
    )
    REFERENCES lms_dep ( -- 부서
      dno -- 부서번호
    );

-- 교육과정
ALTER TABLE lms_lec
  ADD CONSTRAINT FK_lms_mgr_TO_lms_lec -- 매니저 -> 교육과정
    FOREIGN KEY (
      mno -- 매니저번호
    )
...
-- 교육과정
ALTER TABLE lms_lec
  ADD CONSTRAINT FK_lms_room_TO_lms_lec -- 교실 -> 교육과정
    FOREIGN KEY (
      rno -- 교실번호
    )
...
-- 수강신청
ALTER TABLE lms_appl
  ADD CONSTRAINT FK_lms_lec_TO_lms_appl -- 교육과정 -> 수강신청
    FOREIGN KEY (
      lno -- 교육과정번호
    )
    REFERENCES lms_lec ( -- 교육과정
      lno -- 교육과정번호
    );
...
-- 교육과정강사
ALTER TABLE lms_lec_tchr
  ADD CONSTRAINT FK_lms_lec_TO_lms_lec_tchr -- 교육과정 -> 교육과정강사
    FOREIGN KEY (
      lno -- 교육과정번호
    )
    REFERENCES lms_lec ( -- 교육과정
...
-- 교육과정강사
ALTER TABLE lms_lec_tchr
  ADD CONSTRAINT FK_lms_tschr_TO_lms_lec_tchr -- 강사 -> 교육과정강사
    FOREIGN KEY (
      tno -- 강사번호
    )
    REFERENCES lms_tschr ( -- 강사
      tno -- 강사번호
    );

-- 교실사진
ALTER TABLE lms_rpho
  ADD CONSTRAINT FK_lms_room_TO_lms_rpho -- 교실 -> 교실사진
    FOREIGN KEY (
      rno -- 교실번호
    )
    REFERENCES lms_room ( -- 교실
...
-- 부서
ALTER TABLE lms_dep
  ADD CONSTRAINT FK_lms_center_TO_lms_dep -- 지점 -> 부서
    FOREIGN KEY (
      cenno -- 지점번호
    )
    REFERENCES lms_center ( -- 지점
